...
genome <- read.table(paste(DATA_ROOT, "Features", "Cmajor.scaffolds.bed", sep = '/'), col.names = c("chr","start","end")) %>%
  mutate(chr = fct_relevel(chr, chrom_order),
         chr = fct_relevel(chr, c("CMU00.1", "CMU00.2", "CMU00.3"), after = Inf)) %>%
  arrange(chr)

genes <- genomicDensity(read.table(paste(DATA_ROOT, "Annotation/Cmajor_v2.2.annotation.longestIsoform.bed.gz", sep = '/')), window.size = 1e6)
pseudogenes <- genomicDensity(read.table(paste(DATA_ROOT, "Pseudogenes/Cmajor.pseudogenes.bed.gz", sep = '/')), window.size = 1e6)
repeats <- genomicDensity(read.table(paste(DATA_ROOT, "Repeats/Cmajor_v2.scaffolds.repeats.composite_masked.bed.gz", sep = '/')), window.size = 1e6)
