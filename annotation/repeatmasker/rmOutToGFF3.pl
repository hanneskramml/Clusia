#!/home/apps/perl/5.36.0/bin/perl
##---------------------------------------------------------------------------##
##  File:
##      @(#) rmOutToGFF3.pl
##  Author:
...
for ( my $i = 0 ; $i < $searchResults->size() ; $i++ ) {
  ...
  # HK: produce valid gff3 files
  if ( $result->getSubjStart() > $result->getSubjEnd() ) {
    $result->setSubjStart(0);
    $result->setSubjEnd(0);
  }
  if ( $result->getSubjStart < 0 or $result->getSubjEnd() < 0 ) {
    $result->setSubjStart(0);
    $result->setSubjEnd(0);
  }
